...
; string to list
(define (encrypt-helper1 plaintext)
  (string->list plaintext)
  )

;char to int
(define (encrypt-helper2 char)
  (char->integer char)
  )

;recursive function
(define (encrypt-helper3 lst key)
  (cond
    [(null? lst) `()]
    [else (cons (mod-exp (encrypt-helper2 (car lst)) (cadr key) (car key)) (encrypt-helper3 (cdr lst) key))])
  )
  
  
;; Type signature: (encrypt string '(int int)) -> int-list
(define (encrypt plaintext key)
   (encrypt-helper3 (encrypt-helper1 plaintext) key)
  )
...
;list to string
(define (decrypt-helper1 s)
  (list->string s)
  )

;integer to char
(define (decrypt-helper2 i)
  (integer->char i)
  )

;recursive function
(define (decrypt-helper3 ciphertext key)
  (if (null? ciphertext)
      `()
      (cons (decrypt-helper2 (mod-exp (car ciphertext) (cadr key) (car key))) (decrypt-helper3 (cdr ciphertext) key))
      )
  )
;; Type signature: (decrypt int-list '(int int)) -> string
(define (decrypt ciphertext key)
  (decrypt-helper1 (decrypt-helper3 ciphertext key))
  )
