...
(define (get-vertices G)
  (if(null? G)
     `()
     (usort (append (cons (caar G) (cdar G)) (get-vertices (cdr G)))))
  )
...
(define (degree G v)
  (cond
    [(null? G) 0]
    [(and (= v (caar G)) (= v (car(cdar G)))) (+ 2 (degree (cdr G) v))]
    [(or (= v (caar G)) (= v (car(cdar G)))) (+ 1 (degree (cdr G) v))]
    [else (degree (cdr G) v)]
    )
)
...
(define (degree-list G v)
  (if (null? v)
      `()
      (cons (degree G (car v)) (degree-list G (cdr v)))
      )
  )
...
(define (even-check v)
  (cond
    [(null? v) #t]
    [(= 1 (modulo (car v) 2)) #f]
    [else (even-check (cdr v))]
    )
  )

(define (all-even? G)
  (even-check (degree-list G (get-vertices G)))
)
...
(define (adjacent G v)
  (cond
    [(null? G) `()]
    [(= v (caar G)) (usort (cons (car(cdar G)) (adjacent (cdr G) v)))]
    [(= v (car(cdar G))) (usort (cons (caar G) (adjacent (cdr G) v)))]
    [else (usort (adjacent (cdr G) v))]
  )
  )
...
(define (connected-helper1 G v)
  (cond
    [(null? v) #t]
    [(equal? (connected-helper2 (get-vertices G) (usort (dfs G `() (car v)))) #t) (connected-helper1 G (cdr v))]
    [else #f]
    )
  )

(define (connected-helper2 v1 v2)
  (cond
    [(and (null? v1) (null? v2)) #t]
    [(or (null? v1) (null? v2)) #f]
    [(= (car v1) (car v2)) (connected-helper2 (cdr v1) (cdr v2))]
    [else #f]
    )
  )

(define (connected? G)
  (connected-helper1 G (get-vertices G))
  )
...
(define (eulerian? G)
  (and (equal? (connected? G) #t) (equal? (all-even? G) #t))
  )