...
(define (intersection LA LB)
  (cond [(null? LA) `()]
        [(null? LB) `()]
        [(equal? (car LA) (car LB)) (make-set (cons (car LA) (intersection (cdr LA) (cdr LB))))]
        [else
         (if (>= (length (intersection LA (cdr LB))) (length (intersection (cdr LA) LB)))
             (make-set (intersection LA (cdr LB)))
             (make-set (intersection (cdr LA) LB))
        )]
  )
  )
...
(define (disjoint? SA SB)
  (if (= 0 (length (intersection SA SB)))
      #t
      #f
      )
  )
